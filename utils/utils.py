...
def extract_info(job_posting):
    ...
    result = {"skills": None, "hourly_rate": None, "budget": None}
    ...
    hourly_rate_match = re.search(
        r"Hourly Range</b>: \$(\d+\.\d+)-\$(\d+\.\d+)", content_field
    )
    ...
    budget_match = re.search(
        r"budget</b>: \$([,\d]+)\s*-?\s*\$?([,\d]*)", content_field, re.IGNORECASE
    )
    if budget_match:
        min_budget = float(budget_match.group(1).replace(",", ""))
        max_budget = (
            float(budget_match.group(2).replace(",", ""))
            if budget_match.group(2)
            else min_budget
        )
        result["budget"] = {"min": min_budget, "max": max_budget}
